...
def str_to_time(timestamp,tzsource=None):
    ...
    try:
        baldDate = parse(timestamp)

    except Exception as e: 
        #print(e)
        return None

    if baldDate.tzinfo != None:
        return baldDate
    else:
        if tzsource == None:
            return baldDate
            ...
            #localize timezone
            zone = pytz.timezone(tzsource)
            newdate = zone.localize(baldDate)
            return newdate
        else:
            newdate = baldDate.replace(tzinfo = tzsource.tzinfo)
            return newdate
...
def daytime(time,daycycle):
    ...
    strDate = time.isoformat()
    dtYear = strDate[0:4]
    dtMonthDay = strDate[5:10]
    
    try:
        sunriseTime  = daycycle[dtYear][dtMonthDay]["sunrise"]
        sunsetTime = daycycle[dtYear][dtMonthDay]["sunset"]
        daycycletz = pytz.timezone(daycycle["timezone"])
    except:
        return None

    if time.tzinfo == None:
        time = daycycletz.localize(time)

    sunriseStrDate = strDate[0:11] + sunriseTime + ":00"
    sunsetStrDate = strDate[0:11] + sunsetTime + ":00"

    sunriseTime = daycycletz.localize(parse(sunriseStrDate))
    sunsetTime = daycycletz.localize(parse(sunsetStrDate))
    
    if sunriseTime < time and time < sunsetTime:
        return True
    return False
