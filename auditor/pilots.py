...
def get_certification(takeoff,student):
    ...
    if student[3] != "" and student[3] != None:
        ...
        elif utils.str_to_time(student[3], "America/New_York") < takeoff:
            return PILOT_NOVICE
...
def has_instrument_rating(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    if student[7] != "" and student[7] != None:
        if utils.str_to_time(student[7]) < takeoff:
            return True
    return False
...
def has_advanced_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    if student[8] != "" and student[8] != None:
        if utils.str_to_time(student[8]) < takeoff:
            return True
    return False
...
def has_multiengine_endorsement(takeoff,student):
    ...
    Precondition: student is 10-element list of strings representing a pilot
    """
    if student[9] != "" and student[9] != None:
        if utils.str_to_time(student[9]) < takeoff:
            return True
    return False
...
def get_best_value(data, index, maximum=True):
    ...
    returnVal = float(data[0][index])
    if maximum:
        for row in data:
            if float(row[index]) > returnVal:
                returnVal = float(row[index])
    else: #minimum
        for row in data:
            if float(row[index]) < returnVal:
                returnVal = float(row[index])
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    day_table = []
    if daytime:
        for row in vfr_table:
            if row[3] == 'Day':
                day_table.append(row)
    else: # Night
        for row in vfr_table:
            if row[3] == 'Night':
                day_table.append(row)
    ...
    if area == 'Pattern':
        for row in day_table:
            if row[2] == "Pattern" or row[2] == "Local" or row[2] =="Any":
                area_table.append(row)
    elif area == 'Practice Area':
        for row in day_table:
            if row[2] == "Practice Area" or row[2] == "Local" or row[2] == "Any":
                area_table.append(row)
    elif area == 'Cross Country':
        for row in day_table:
            if row[2] == "Cross Country" or row[2] == "Any":
                area_table.append(row)
    ...
    if cert == 3:
        for row in area_table:
            if row[0]=='Student' or row[0]=='Certified' or row[0]=='50 Hours':
                results_table.append(row)
    elif cert ==2:
        for row in area_table:
            if row[0]=='Student' or row[0]=='Certified':
                results_table.append(row)
    elif cert ==1:
        for row in area_table:
            if row[0] == 'Student':
    ...
    ceiling = get_best_value(results_table, 4, False)
    visibility = get_best_value(results_table, 5, False)
    wind = get_best_value(results_table, 6)
    crosswind = get_best_value(results_table, 7)
